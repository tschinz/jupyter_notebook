...
from poisson_disk.datastructures import RandomQueue
from poisson_disk.enhanced_grid import int_point_2d
from poisson_disk.enhanced_grid import int_point_3d
from poisson_disk.enhanced_grid import Grid2D
from poisson_disk.enhanced_grid import Grid3D
from poisson_disk.enhanced_grid import ListGrid2D
from poisson_disk.enhanced_grid import ListGrid3D
...
##@brief The square of the distance between the given points
def sqr_dist_3d(x0_y0_z0, x1_y1_z1):
    (x0, y0, z0) = x0_y0_z0
    (x1, y1, z1) = x1_y1_z1
...
def sample_poisson_uniform(width, height, r, k):
    ...
    def generate_random_around(x_y, r):
        (x, y) = x_y
        rr = uniform(r, 2*r)
        rt = uniform(0, 2*pi)
...
def sample_poisson_3d(width, height, depth, r_grid, k):
    ...
    # Generates a point randomly selected around
    # the given point, between r and 2*r units away.
    def generate_random_around(x_y_z, r):
        (x, y, z) = x_y_z
        rr = uniform(r, 2*r)
        rs = uniform(0, 2*pi)
        rt = uniform(0, 2*pi)
