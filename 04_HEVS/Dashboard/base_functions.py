...
###############################################################################
# Plot figures
def stacked_bar_plot(df, df_col, projectList, projectListColumns, projectConf, outputGraphDir, ext_file):
  fig = go.Figure()
  fig = make_subplots(specs=[[{"secondary_y": True}]])
  title = datetime.date.today().strftime("%Y.%m.%d") + " " + projectConf[0] + " " + filterRows(projectList, [projectListColumns[0], [projectConf[0]]])[projectListColumns[1]].iloc[0]
  title_long = datetime.date.today().strftime("%d.%m.%Y") + " - " + projectConf[0] + " - " + filterRows(projectList, [projectListColumns[0], [projectConf[0]]])[projectListColumns[2]].iloc[0]
  # Add memebre data
  for member in df[df_col[1]].unique():
    labels = df.loc[df[df_col[1]] == member][df_col[0]]
    values = df.loc[df[df_col[1]] == member][df_col[4]]
    fig.add_trace(go.Bar(x=labels, y=values, name=member, texttemplate = "%{label}", textposition = "inside"), secondary_y=False)
  ...
  labels = df[df_col[0]]
  values = df[df_col[7]]
  fig.add_trace(go.Scatter(x=labels, y=values, name=df_col[7]), secondary_y=True)
  values = df[df_col[8]]
  fig.add_trace(go.Scatter(x=labels, y=values, name=df_col[8]), secondary_y=True)
  values = df[df_col[9]]
  fig.add_trace(go.Scatter(x=labels, y=values, name=df_col[9]), secondary_y=True)
  
  # Update Graph
  fig.update_layout(barmode='stack',
                    xaxis={'categoryorder':'total descending', 'type': 'date'},
                    title = {'text': title_long, 'x': 0.5, 'y': 0.9},
                    xaxis_title = df_col[0],
                    yaxis_title = df_col[4],
                   )
  fig.update_yaxes(title_text=df_col[4], secondary_y=False)
...
def plot_figure(graphFilename, fig, overrideNotebookGraphicInteraction=None, height=350):
    ...
    Raises:
        None
    """
    if (notebookGraphicOutputs == GraphOutputOption('extFile')) or (notebookGraphicOutputs == GraphOutputOption('both')):
    ...
    if (notebookGraphicOutputs == GraphOutputOption('inline')) or (notebookGraphicOutputs == GraphOutputOption('both')):
        choice = None
        if overrideNotebookGraphicInteraction == None:
            if notebookGraphicInteraction == GraphInteractionOption('static'):
                choice = 'static'
            elif notebookGraphicInteraction == GraphInteractionOption('interactive'):
                choice = 'interactive'
        elif overrideNotebookGraphicInteraction == GraphInteractionOption('static'):
            choice = 'static'
        elif overrideNotebookGraphicInteraction == GraphInteractionOption('interactive'):
            choice = 'interactive'
        else:
            choice = 'static'
